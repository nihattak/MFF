...
#'
#' @export
tune.mff <- function(pred_matrix, y, max_c = 5, m_seq = seq(1.1, 3, by = 0.1),eta_seq = seq(1.1, 3, by = 0.4), iter.max = 1000, nstart = 100,
  ...
  for (i in 1:nrow(search_grid)) {
    ...
    if (current_metric < best_method) {
      ...
      best_params <- params
      best_membership <- mff_result$weight_matrix
  ...
  best_weight <- best_membership[, idx]
